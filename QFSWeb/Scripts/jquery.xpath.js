...
(function () {
    ...
    cDOMAdapter.prototype.isNode = function (oNode) {
        return cDOMAdapter.isNode(oNode);
    };
    ...
    cDynamicContext.prototype.clone = function (oItem, nPosition) {
        var other = new cDynamicContext(this.staticContext, oItem || this.item, this.cloneScope(), this.DOMAdapter);
        other.position = nPosition;
        other.stack = this.cloneStack();

        return other;
    };

    // For setting a variable value from outside this library
    cDynamicContext.prototype.setVariable = function (sName, vValue) {
        var xsValue = cStaticContext.js2xs(vValue);
        this.pushVariable(sName, xsValue);
    };

    cDynamicContext.prototype.cloneScope = function () {
        var newScope = {}, prop;

        for (prop in this.scope) {
            newScope[prop] = this.scope[prop];
        }

        return newScope;
    };

    cDynamicContext.prototype.cloneStack = function () {
        var newStack = {}, prop;

        for (prop in this.stack) {
            newStack[prop] = this.stack[prop].slice(0);
        }

        return newStack;
    };

    cDynamicContext.prototype.hasVariable = function (sUri) {
        return this.scope.hasOwnProperty(sUri);
    };

    cDynamicContext.prototype.getVariable = function (uri) {
        return this.scope[sUri];
    };
    ...
    cStaticContext.js2xs = function (vItem) {
        ...
        else if (typeof vItem != "string" && vItem.length !== undefined) {
            var oSequence = [];
            for (var i = 0; i < vItem.length; ++i) {
                var converted = cStaticContext.js2xs(vItem[i])
                oSequence.push(converted);
            }
            vItem = oSequence;
        }
        else if (cDOMAdapter.isNode(vItem)) {
    ...
    cExpr.prototype.evaluate = function (oContext) {
        ...
        for (var nIndex = 0, nLength = this.items.length; nIndex < nLength; nIndex++) {
            var result = this.items[nIndex].evaluate(oContext);
            oSequence = hStaticContext_operators["concatenate"].call(oContext, oSequence, result);
        }
        return oSequence;
    };
    ...
    cVarRef.prototype.evaluate = function (oContext) {
        ...
        if (oContext.scope.hasOwnProperty(sUri)) {
            var val = oContext.scope[sUri];
            // [Jimmy] Removed the brackets around val since we are currently storing variables as sequences
            return val;
        }
    ...
    function fFunction_sequence_toString(oItem, context) {
        return oItem == null ? new cXSString('') : cXSString.cast(fFunction_sequence_atomize([oItem], context)[0]);
    }
    ...
    fStaticContext_defineSystemFunction("string", [[cXTItem, '?', true]], function (oItem) {
        ...
        return fFunction_sequence_toString(oItem, this);
    });
    ...
    fStaticContext_defineSystemFunction("generate-id", [[cXTNode, '?', true]], function () {
        return new cXSString("");
    });
...
cAttr.prototype.ownerElement = null;
    ...
    function fXPath_node_from_query(oQuery) {
        var oNode = oQuery[0];
    ...
    function fXPath_static_context_from_node(oNode) {
        ...
        return oStaticContext;
    }

    function fXPath_get_adapter(oStaticContext) {
        var oAdapter = oL2DOMAdapter;
    ...
    function fXPath_parse(oQuery, sExpression, fNSResolver) {
        if (typeof sExpression == "undefined" || sExpression === null)
            sExpression = '';

        var oNode = fXPath_node_from_query(oQuery);

        var oStaticContext = fXPath_static_context_from_node(oNode);
    ...
    function fXPath_evaluate_and_package(oExpression, oContext) {
        var aSequence, oSequence = new cQuery;

        aSequence = oExpression.evaluate(oContext);
        var oAdapter = oContext.DOMAdapter;
    ...
    function fXPath_evaluate(oQuery, sExpression, fNSResolver) {
        ...
        var oExpression = sExpression instanceof cExpression ? sExpression : fXPath_parse(oQuery, sExpression, fNSResolver);

        var oAdapter = fXPath_get_adapter(oStaticContext);

        var dc = new cDynamicContext(oStaticContext, oNode, null, oAdapter);

        return fXPath_evaluate_and_package(oExpression, dc);
    };

    var oObject = {};
    oObject.xpath = function (oQuery, sExpression, fNSResolver) {
        return fXPath_evaluate(oQuery instanceof cQuery ? oQuery : new cQuery(oQuery),
                               sExpression, fNSResolver);
    };
    ...
    oObject = {};
    oObject.xpathParse = function (sExpression, fNSResolver) {
        return fXPath_parse(this, sExpression, fNSResolver);
    };
    cQuery.extend(cQuery.prototype, oObject);

    oObject = {};
    oObject.makeContext = function (dom) {
        var static = fXPath_static_context_from_node(dom);
        var adapter = fXPath_get_adapter(static);
        return new cDynamicContext(static, dom, null, adapter);
    };
    cQuery.extend(cQuery, oObject);

    oObject = {};
    oObject.xpathWithContext = function (expr, context) {
        return fXPath_evaluate_and_package(expr, context);
    };
    cQuery.extend(cQuery, oObject);

    oObject = {};
    oObject.xpBooleanValue = function (context) {
        var item = this[0];
        return item &&
               (
               context.DOMAdapter.isNode(item) ||
               typeof item == "boolean" ||
               typeof item == "number" ||
    ...
    oObject = {};
    oObject.xpStringValue = function (context) {
        var item = this[0];
        if (!item) {
            return "";
        } else if (context.DOMAdapter.isNode(item)) {
            var tc = item.textContent;
            return (tc || tc === "") ? tc : item.nodeValue;
        } else {
